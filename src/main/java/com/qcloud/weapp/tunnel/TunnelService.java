...
public class TunnelService {
	...
	private static final Logger logger = LoggerFactory.getLogger(TunnelService.class);
	 ...
	 */
	public JSONObject handle(TunnelHandler handler, TunnelHandleOptions options) throws ConfigurationException {
		if ("GET".equals(request.getMethod().toUpperCase())) {
			return handleGet(handler, options);
		}
		if ("POST".equals(request.getMethod().toUpperCase())) {
			return handlePost(handler, options);
		}
		return null;
	}
	 ...
	 */
	private JSONObject handleGet(TunnelHandler handler, TunnelHandleOptions options) throws ConfigurationException {
		Tunnel tunnel = null;
		UserInfo user = null;

		logger.info("处理handle get请求");
		...
		try {
			...
			logger.info("receiveUrl====================================" + receiveUrl);
...
//			writeJson(getJsonForError(e));
		...
		logger.info("tunnel 信息为：" + tunnel);

		JSONObject result = new JSONObject();
		try {
			result.put("connectUrl", tunnel.getConnectUrl());
		} catch (JSONException e) {
			e.printStackTrace();
		}
//		writeJson(result);
		...
		System.out.println("tunnel connecturl===========" + tunnel.getConnectUrl());
	...
	private JSONObject handlePost(TunnelHandler handler, TunnelHandleOptions options) throws ConfigurationException {
		...
		try {
			...
			e.printStackTrace();
//			writeJson(getJsonForError(e));
			return getJsonForError(e);
		}

		logger.info("requestContent 是：" + requestContent);
